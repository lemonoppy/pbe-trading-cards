...
async function main() {
  ...
  await Promise.all(
    records.map(async (record, index) => {
      const query = SQL`SELECT cardID, player_name, sub_type FROM pbe_cards WHERE playerID=${record.playerId} AND card_rarity=${record.rarity} AND sub_type=${record.subType}`

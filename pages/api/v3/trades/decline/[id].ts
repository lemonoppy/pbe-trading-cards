...
export default async function declineTradeEndpoint(
  req: NextApiRequest,
  ...
  if (req.method === POST) {
    ...
    const tradeResult = await cardsQuery<{
      initiatorid: string
      recipientid: string
    }>(
      SQL`
      SELECT initiatorID, recipientID
      FROM pbe_trades WHERE tradeID=${parseInt(tradeID)}`
    )
    ...
    if (
      ...
      res
        .status(StatusCodes.BAD_REQUEST)
        .end('You are not authorized to decline this trade')
      return
    }

    const queryResult = await cardsQuery(
      SQL`CALL decline_trade(${tradeID}, ${req.cookies.userid});`
    )

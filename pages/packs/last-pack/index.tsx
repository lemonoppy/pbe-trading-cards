...
import { rarityDefinitions } from '@constants/rarity-map'
...
// Hex codes for legacy styling (kept for backward compatibility)
const HexCodes = {
  Ultimus: '#E0115F',
  Diamond: '#459F94',
  Gold: '#BD8700',
...
const LastOpenedPack = () => {
  ...
  const packLabel = getPackLabel(type)
  ...
  const OpenNextPack = async (pack: UserPacks) => {
    ...
    if (!pack) {
      toast({
        ...
        description: `You don't have any ${packLabel} packs to open.`,
        ...errorToastOptions,
      })
      return
    }

    // Reset previous mutation state before opening new pack
    resetOpenPack()
    openPack({ packID: pack.packid, packType: pack.packtype })
  }
  ...
  // Use shared rarity definitions from constants
  const cardRarityShadows = Object.values(rarityDefinitions).map((def) => ({
    id: def.label,
    emoji: def.emoji,
    color: def.color,
  }))
  ...
  const shareMessage = () => {
    ...
    const specialCardMessages = latestPackCards
      .filter((card) => Number(card.totalCardQuantity) === 1 || Number(card.quantity) === 1)
      .map((card) => {
        if (Number(card.totalCardQuantity) === 1) {
          return `First pull of ${card.sub_type} ${card.player_name}! 🌟`
        }
        if (Number(card.quantity) === 1 && Number(card.totalCardQuantity) > 1) {
          return `My first ${card.sub_type} ${card.player_name}! 🆕`
        }
        return null
      })
      .filter(Boolean)
    ...
    const message = `I opened pack #${packID} \n ${emojis}${specialCardText}\nCheck out my cards: https://www.dottstradingcards.com/packs/${packID}`

    navigator.clipboard.writeText(message).then(() => {
      toast({
        title: 'Copied to clipboard',
        description: 'Share with friends on discord!',
        ...successToastOptions,
      })
    })
  }
  ...
  useEffect(() => {
    const handlePackOpenSuccess = async () => {
      if (useOpenPackIsSuccess && user?.uid) {
        ...
        await queryClient.refetchQueries(getLatestPackCardsKey(user.uid))
        ...
        toast({
          ...
          description: 'Your new pack is ready to view!',
          ...successToastOptions,
        })
      }
    }

    handlePackOpenSuccess()
  }, [useOpenPackIsSuccess, user?.uid, loggedInUID, type, queryClient, toast])
      ...
      <div className="h-full w-full m-1">
        ...
        <div className="flex items-center space-x-2">
          ...
          {firstPack && (
            ...
            >
              {packs?.length} {packLabel} {packs?.length === 1 ? 'Pack' : 'Packs'}
            </Badge>
          )}
        </div>
          ...
          {firstPack ? (
            <Tooltip label="Open a new pack">
              ...
              >
                ...
                Open Another {packLabel} Pack
              </Button>
            </Tooltip>
          ) : (
            <Button className="!text-xs sm:!text-lg" isDisabled>
              No More {packLabel} Packs to Open
            </Button>
          )}
          <Button
            ...
            <div className="px-8 flex h-full flex-col sm:grid sm:grid-cols-3 lg:grid-cols-6 gap-3 overflow-x-auto py-6 no-scrollbar">
              {latestPackCards.map((card, index) => (
                ...
                >
                  ...
                  {revealedCards.includes(index) &&
                    Number(card.quantity) === 1 && ( // Check for quantity and show badge
                      <Badge
                        colorScheme="green"
                        variant="outline"
                        border="2px solid"
                  ...
                  {revealedCards.includes(index) &&
                    Number(card.totalCardQuantity) === 1 && ( // If first ever card thats been opened
                      <Tooltip

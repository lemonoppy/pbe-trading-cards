...
export default ({ tradeid }: { tradeid: string }) => {
  ...
  const {
    initiatorCards,
    initiatorCardsInfo,
    initiatorSubTypeInfo,
    recipientCards,
    recipientCardsInfo,
    recipientSubTypeInfo,
  }: {
    initiatorCards: TradeDetails[]
    initiatorCardsInfo: Record<string, number>
    initiatorSubTypeInfo: Record<string, number>
    recipientCards: TradeDetails[]
    recipientCardsInfo: Record<string, number>
    recipientSubTypeInfo: Record<string, number>
  } = useMemo(() => {
    if (tradeInformationIsLoading || !tradeInformation || tradeInformation.length === 0) {
      return {
        initiatorCards: [],
        recipientCards: [],
        initiatorCardsInfo: {},
    ...
    const tempInitiatorSubTypeCounts: Record<string, number> = {}
    const tempRecipientSubTypeCounts: Record<string, number> = {}
    ...
    tradeInformation?.forEach((tradeDetail) => {
      if (tradeDetail.fromid == tradeDetail.initiatorid) {
        ...
        tempInitiatorSubTypeCounts[tradeDetail.sub_type] =
          (tempInitiatorSubTypeCounts[tradeDetail.sub_type] || 0) + 1
      } else {
        ...
        tempRecipientSubTypeCounts[tradeDetail.sub_type] =
          (tempRecipientSubTypeCounts[tradeDetail.sub_type] || 0) + 1
      }
    })
    ...
    return {
      ...
      initiatorSubTypeInfo: tempInitiatorSubTypeCounts,
      recipientCardsInfo: tempRecipientCounts,
      recipientSubTypeInfo: tempRecipientSubTypeCounts,
    }
  ...
  const { payload: initiatorUser } = query<UserData>({
    queryKey: ['user', String(initiatorCards[0]?.fromid)],
    queryFn: () =>
      axios({
        method: GET,
  ...
  const { payload: recipientUser } = query<UserData>({
    queryKey: ['user', String(recipientCards[0]?.fromid)],
    queryFn: () =>
      axios({
        method: GET,
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 sm:py-8">
        ...
        <div className="hidden sm:flex gap-8">
          <PendingTradeSection
            ...
            cardsSubTypeInfo={initiatorSubTypeInfo}
          ...
          <PendingTradeSection
            ...
            cardsSubTypeInfo={recipientSubTypeInfo}
            cardsInTrades={cardsInTrades}
            cardsInTradesLoading={cardsInTradesLoading}
            loggedInID={loggedInUser?.uid}
          />

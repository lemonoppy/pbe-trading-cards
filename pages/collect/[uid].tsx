...
const SORT_OPTIONS: OwnedCardSortOption[] = [
  ...
  },
  {
    ...
    label: 'Rarity',
    sortLabel: (direction: SortDirection) =>
      direction === 'DESC' ? '(Common->Mythic)' : '(Mythic->Common)',
  },
  {
    value: 'season',
    label: 'Draft Season',
...
const CardGrid = memo(({
  ...
  isLoading: boolean
  cards: OwnedCard[] | undefined
  onCardClick: (card: OwnedCard) => void
}) => {
  return (
    <SimpleGrid
        ...
        : cards?.map((card, index) => (
            ...
            >
              ...
              <Badge className="z-30 absolute top-0 left-0 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-white transform -translate-x-1/4 -translate-y-3/4 bg-neutral-800 rounded-full">
                {`${card.sub_type} (${card.card_rarity}) - ${card.quantity}`}
              </Badge>
            </div>
          ))}
    </SimpleGrid>
...
export default ({ uid }: { uid: string }) => {
  ...
  const [sortColumn, setSortColumn] = useState<OwnedCardSortValue>('card_rarity')
  ...
  useEffect(() => {
    if (payload) {
      ...
      setTotalOwnedCards(payload.totalOwned ?? 0)
    }
    if (subTypeData && subTypeData.length === 0 && subType.length > 0) {
      setSubType([])
    }
  }, [payload, subTypeData, subType])
  ...
  const handleCardClick = useCallback((card: OwnedCard) => {
        ...
        <AccordionItem className="overflow-visible">
          ...
          <AccordionPanel pb={4} className="overflow-visible">
            <VStack spacing={4} align="stretch" className="px-4">
              ...
              {showNotOwnedCards && totalCards > 0 && (
                <div className="relative z-0">
                  Owned Cards: {totalOwnedCards ?? 0} / {totalCards} (
                  {(((totalOwnedCards ?? 0) / totalCards) * 100).toFixed(2)}%)
                </div>
              )}

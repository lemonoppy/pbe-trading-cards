...
export const SessionProvider = ({
  ...
  const handleLogin = useCallback(
    async (username: string, password: string) => {
      ...
      if (response.status !== StatusCodes.OK || response.data.status === 'error') {
        throw new Error(response.data.message || 'Login failed')
      }

      if (!response.data.payload) {
        console.error('Login response missing payload:', response.data)
      ...
      setRefreshToken(response.data.payload.refreshToken, 30)
      setUserID(response.data.payload.userid, 30)
      setIsLoading(false)
  ...
  const handleLogout = useCallback(() => {
    ...
    deleteUserID()
    // Clear all React Query cache to prevent old user data from showing
    queryClient.clear()
  }, [deleteRefreshToken, deleteUserID, queryClient])

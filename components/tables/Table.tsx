          ...
          <tbody className="relative table-row-group bg-primary align-middle">
            {table.getRowModel().rows.map((row) => (
              <tr key={row.id} className="hover:bg-blue600/10">
                {tableBehavioralFlags.stickyFirstColumn && (
                  <th className="sticky left-0 table-cell border-t border-t-table bg-primary text-left font-mont font-normal text-secondary">
                    {flexRender(
                ...
                {row.getVisibleCells().map((cell, i) => {
                  ...
                  return (
                    <td
                      key={cell.id}
                      className={classnames(
                        'whitespace-nowrap border-t border-t-table p-2 text-center font-mont text-xs md:text-lg text-secondary',

...
import { LEAGUE_OPTIONS, DEFAULT_LEAGUE_ALL } from 'lib/constants'
...
export default function NewTrade({
  ...
  const [leagueID, setLeagueID] = useState<string[]>(DEFAULT_LEAGUE_ALL)
      ...
      >
        ...
        <DrawerContent overflow="hidden" overflowY="scroll">
          ...
          <DrawerBody className="bg-primary p-4">
            ...
            <SimpleGrid
              className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-3 bg-primary"
                ...
                : selectedUserCards?.rows.map((card, index) => {
                    ...
                    return (
                      <div
                        ...
                        className="relative transition ease-linear shadow-none hover:scale-105 hover:shadow-xl cursor-pointer"
                        onKeyDown={(e) => {
                      ...
                      >
                        ...
                        {!selectedUserCardsIsLoading && (
                          <Badge className="z-30 absolute top-0 left-0 inline-flex items-center justify-center px-1 py-0.5 text-[0.65rem] font-bold leading-none text-primary transform -translate-x-1/4 -translate-y-1/2 bg-neutral-800 rounded-full">
                            {card.card_rarity}
                          </Badge>
                        )}
                      </div>
